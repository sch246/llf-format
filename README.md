...
v0.4 草案，欢迎讨论。完整规范见 [SPEC.md](SPEC.md)；命名与版本变化见 SPEC.md 第 12 节。

## 相关项目

[NestedText](https://nestedtext.org) 同样用行前缀免除转义，也把标量类型交给应用层。LLF 的结构更显式（`{}` / `[]` 必须写出，空容器无歧义），并增加了流式语义与截断检测，定位于机器生成。
